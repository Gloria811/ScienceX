// This example allow to control a motor with EPOS4 controler.
//
// EPOS have to be previously configured with Maxon software to integrate all motor and encoder caracteristics.
// All cyclic mode and profile position are usable.
// To use this example execute:
//
// sudo ./epos4-example 'network_interface_name' 'control_mode' 'target_value'
//
// 'network interface name': network interface where EPOS is connected (ex: eth0 )
// 'control_mode': desired control mode
//       - 'profile_position': profile position mode in relative positionning (unit: qc)
...
void* cyclic_Task(void* arg){
  ...
  while(!stop){
    ...
    bool wkc = ethercat_master.next_Cycle();
...
int main(int argc, char* argv[])
{
  ...
  std::string input_control_mode = argv[2];
  std::string input_target = argv[3];
